...
public class ReceipeDetailStepFragmentTest {
        ...
        public void checkDisplayedInDynamicallyCreatedFragment() {
            ReceipeDetailStepFragment fragment = new ReceipeDetailStepFragment();
            mActivityTestRule.getActivity().getSupportFragmentManager().beginTransaction()
                    .add(R.id.step_detail_container, fragment).commit();
            //onView(ViewMatchers.withId(R.id.playerView)).check(matches(not(isDisplayed())));
            onView(ViewMatchers.withId(R.id.tv_receipedetail)).check(matches((isDisplayed())));
        }
