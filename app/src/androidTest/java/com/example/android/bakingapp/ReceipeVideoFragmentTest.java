...
public class ReceipeVideoFragmentTest {
        ...
        public void checkTextDisplayedInDynamicallyCreatedFragment() {
            ReceipeVideoFragment fragment = new ReceipeVideoFragment();
            mActivityTestRule.getActivity().getSupportFragmentManager().beginTransaction()
                    .add(R.id.video_container, fragment).commit();
            //onView(ViewMatchers.withId(R.id.playerView)).check(matches(not(isDisplayed())));
            onView(hasMinimumChildCount(1));
        }
        }
